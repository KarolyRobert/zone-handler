...
describe("dns_record",() => {
    ...
    test("dns_record nedd to have add update delete method", () => {
    ...
    describe("dns_record.update need to return an array of proper command to delete and add record with nsupdate except the case of SOA type record when need to return only a proper command to add the updated SOA record.",() => {

        let raw = {name:'www',ttl:6300,type:'A',data:'127.0.0.1'}
        let update_raw = {name:'update',ttl:3200,type:'CNAME',data:'www.example.com.'}
        let record = dns_record(raw,'example.com');
        test("simple case of update.",() => {
            expect(record.update_command(dns_record(update_raw,'example.com'))).toStrictEqual(['update delete www.example.com. 6300 A 127.0.0.1','update add update.example.com. 3200 CNAME www.example.com.']);
        });
        ...
        test("case of SOA reecord.", () => {
            ...
            record = dns_record(raw,'example.com');
            expect(record.update_command(dns_record(update_raw,'example.com'))).toStrictEqual(['update add example.com. 21600 SOA ns1.example.com. hostmaster.example.com. 2015081501 43200 7200 2419200 600']);
        });
    });
